...
const cty = document.getElementById("city-name");
const cntry = document.getElementById("country");

const xhr = new XMLHttpRequest();

function fetchData({ lat, lng, city}) {
...
function render({current_weather}, city) {
  console.log(current_weather.temperature, city)
  temp.innerText = current_weather.temperature
  cty.innerText = city
}

inp.addEventListener("change", () => {
  if (inp.value.length < 3) return;
  fetchData(JSON.parse(inp.value));
});
jsonData.forEach((i) => {
  let elem = document.createElement("option");
  elem.innerText = i.city;
  elem.setAttribute("value", `${JSON.stringify({ lat: i.lat, lng: i.lng, city: i.city, country: i.country })}`);
  inp.append(elem);
});
